    ...
    public HashMap<String, String> getEnvironment(@NonNull Context currentPackageContext, boolean isFailSafe) {
        ...
        if (!isFailSafe) {
            ...
            if (TermuxBootstrap.isAppPackageVariantAPTAndroid5()) {
                ...
                // Termux binaries on Android 7+ normally rely on DT_RUNPATH, so LD_LIBRARY_PATH should be unset by default.
                // However, since we changed the package name from com.termux to com.anroot, the DT_RUNPATH in the
                // bootstrap ELF binaries points to /data/data/com.termux/files/usr/lib which doesn't exist.
